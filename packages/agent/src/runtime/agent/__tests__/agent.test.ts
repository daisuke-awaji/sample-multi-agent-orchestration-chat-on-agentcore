...
jest.unstable_mockModule('../../../config/index.js', () => ({
  ...
  config: {
    ...
    AGENTCORE_MEMORY_ID: undefined,
  },
  WORKSPACE_DIRECTORY: '/tmp/ws',
  buildSystemPrompt: mockBuildSystemPrompt,
  generateDefaultContext: jest.fn().mockReturnValue(''),
}));
...
jest.unstable_mockModule('../../../config/bedrock-model.js', () => ({
  createBedrockModel: mockCreateBedrockModel,
}));

jest.unstable_mockModule('../../../services/session/cache-point-appender.js', () => ({
  CachePointAppender: jest.fn().mockImplementation(() => ({
    apply: jest.fn((messages: unknown[]) => messages),
  })),
}));

jest.unstable_mockModule('../../../lib/context/request-context.js', () => ({
