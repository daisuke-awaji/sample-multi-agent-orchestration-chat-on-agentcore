 ...
 *
 * Pure type definitions with no implementation logic.
 * Located in models/ (L1) so that lower layers (context, middleware)
 * can depend on these types without violating the layered architecture.
 *
 * Higher layers (session, services, agent, handlers) may import
 * from here directly or via the re-export barrel at session/types.ts.
 */
