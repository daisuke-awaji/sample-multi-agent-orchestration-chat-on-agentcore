...
export async function pingCommand(
  ...
  try {
    ...
    console.log(
      `${chalk.green("✅")} ステータス: ${chalk.bold(result.ping.status)}`
    );
    console.log(
    ...
    console.log(
      `${chalk.blue("📝")} サービス: ${chalk.bold(result.serviceInfo.service)}`
    );
    console.log(
      `${chalk.blue("🏷️")} バージョン: ${chalk.bold(
        result.serviceInfo.version
      )}`
    );
    console.log(
      `${chalk.blue("🚀")} ステータス: ${chalk.bold(result.serviceInfo.status)}`
    );

    console.log("");
    console.log(chalk.bold("⚡ パフォーマンス:"));
    console.log(
      `${chalk.yellow("⏱️")} 接続時間: ${chalk.bold(result.connectionTime)}ms`
    );
